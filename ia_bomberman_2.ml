...
let a_star grille (joueur : joueur) destination =
  ...
  try
    while not visite.(x).(y) do
      ...
      if !chemin = [] then (x0, y0) else List.hd !chemin ;
    with
    | Exit -> (-1, -1) ;;
...
let gain_max_bombe grille (joueur : joueur) positions_accessibles =
  ...
  List.sort comparaison (List.filter (fun (x, y) -> gain_bombe x y <> 0.) positions_accessibles) ;;

let esquivable temps_jeu grille (joueur : joueur) positions_safe bombe =
  let position_safe = List.filter (fun (x, y) -> not (position_dangereuse x y [|bombe|])) positions_safe in
  let position_safe_atteignables = List.filter (fun (x, y) -> (distance_joueur joueur x y) + 1 < int_of_float ((bombe.instant -. temps_jeu) /. (0.9**(float_of_int joueur.vitesse)))) position_safe in
  position_safe_atteignables <> [] ;;



let position_a_bomber temps_jeu grille (joueur : joueur) positions_accessibles positions_safe =
  let position_a_bomber = gain_max_bombe grille joueur positions_accessibles in
  let position_safe_a_bomber = List.filter (fun (x, y) -> esquivable temps_jeu grille joueur positions_safe {ligne = x ; colonne = y ; taille = joueur.deflagration ; instant = temps_jeu +. 5.5}) position_a_bomber in
...
let decision_strategique instant (num_joueur : int) (grille : grille) (bombes : bombes array) (joueurs : joueur array) powerups =
  ...
  (* 3. Identifier un emplacement stratégique pour poser une bombe *)
    ...
    let positions_accessibles_sans_bombes = List.filter (fun (x, y) -> List.for_all (fun bombe -> bombe.ligne <> x && bombe.colonne <> y) (Array.to_list bombes)) positions_accessibles in
    let pos_bombe = position_a_bomber instant grille joueur positions_accessibles_sans_bombes positions_safe in
